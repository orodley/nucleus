		...
		uint32_t num_captures, nuc_val **captured_vars)
{
	...
	memcpy(&lambda->captures, captured_vars, num_captures * sizeof *captured_vars);
...
Lambda *rt_proxy_for_lambda(Lambda *lambda, void *replacement_func_pointer)
{
	...
	memcpy(&proxy->captures, &lambda->captures, lambda->num_captures * sizeof(nuc_val *));
	proxy->captures[lambda->num_captures] = (nuc_val *)lambda->function;
